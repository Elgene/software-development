...
public class Board {
	...
	private int[] image;//The original image on the board
	...
	public boolean isFinished() {
		List<Integer> ListOfImages = new ArrayList<Integer>(); //save all the loaded picture
		for(int i = 0;i < cells.length;i++) {// check each cell
			Cell specficCell = cells[i];
			if(specficCell!=null) {
				for(int j = 0 ;j < specficCell.getImage().length;j++) {//traverse every image
					ListOfImages.add(specficCell.getImage()[j]);
				}
			}
		}
		for(int i = 0;i < ListOfImages.size();i++) {
			if(image[i]!=ListOfImages.get(i)) return false;//If they are not equal return false
		}
		if(cells[cells.length-1]!=null) return false;//The last image must be null
		return true;
	...
	public void swap(Location l1, Location l2) {
		...
		Cell temporary = cells[id1];
		cells[id1] = cells[id2];
		cells[id2] = temporary;
	}
	//CHANGES HERE
	public void rotate(Cell cell) {// change to clockwise of each cell every images
		// Doing auto
		int tempArray0 = cell.getImage()[0];
		int tempArray1 = cell.getImage()[1];
		int tempArray2 = cell.getImage()[2];
		int tempArray3 = cell.getImage()[3];
		cell.getImage()[0]=tempArray2;
		cell.getImage()[1]=tempArray0;
		cell.getImage()[2]=tempArray3;
		cell.getImage()[3]=tempArray1;
	}

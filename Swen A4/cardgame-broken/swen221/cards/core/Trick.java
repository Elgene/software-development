...
public class Trick implements  Cloneable, Serializable {
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		List<Card> listOfCards = getCardsPlayed();
		Player.Direction playerDirection = lead;
		for(int i = 0; i < listOfCards.size(); i++){
			playerDirection = playerDirection.next();
		}

		if(playerDirection != p.direction)
			throw new IllegalMove(" Invalid move: it's not your turn");

		if(!p.getHand().contains(c))
			throw new IllegalMove(" Invalid move:No card found in your hand");

		if(lead != p.direction){
			Card.Suit cardSuit = cards[0].suit();
			if(!p.getHand().matches(cardSuit).isEmpty() && c.suit() != cardSuit)
				throw new IllegalMove(
						"Invalid move: the card is not the same as the leader!");
		}
